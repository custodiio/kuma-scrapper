"""
Script Patch v5:
- Limpa o JavaScript do openSeriesModal utilizando Template Literals (backticks)
- Garante que a abertura do modal #seriesModal funciona 100% no clique do usuário sem erro de sintaxe JS
"""
...
async def index(
    ...
    if tab == "collections":
        ...
        <div class="vitrine-container">
            <h2 class="section-title">🍿 Coleções Ativas do Douyin (Clique para Ver Episódios)</h2>
...
<body>
    ...
    <!-- MODAL DETALHES DA COLEÇÃO E EPISÓDIOS -->
    <div id="seriesModal" class="modal-overlay">
        <div class="modal-container" style="max-width:860px; width:90%; padding:24px;">
            <span class="modal-close" onclick="closeSeriesModal()">&times;</span>
            <div id="modalHeroContent" style="display:flex; gap:20px; margin-bottom:24px; border-bottom:1px solid #232736; padding-bottom:20px;"></div>
            <h3 style="margin:0 0 16px 0; color:#fff; font-size:1.1rem;">📺 Episódios Mapeados:</h3>
            <div id="modalEpisodesList" style="display:flex; flex-direction:column; gap:10px; max-height:450px; overflow-y:auto; padding-right:8px;"></div>
        </div>
    </div>
    ...
    <script>
        ...
        function openSeriesModal(mixId) {{
            fetch(ROOT_PATH + '/api/douyin/collections/' + mixId)
                .then(r => r.json())
                .then(data => {{
                    if (!data.ok) {{ alert('Erro ao carregar coleção.'); return; }}
                    ...
                    `;
                    document.getElementById('modalHeroContent').innerHTML = heroHtml;
                    ...
                    document.getElementById('modalEpisodesList').innerHTML = epsHtml || '<p style="color:#aaa;">Nenhum episódio cadastrado nesta coleção.</p>';
                    document.getElementById('seriesModal').style.display = 'flex';
                }})
                .catch(err => alert('Erro ao abrir coleção: ' + err));
        }}
        ...
        function toggleAutoposting(mixId) {{
            fetch(ROOT_PATH + '/api/douyin/collections/' + mixId + '/toggle-autoposting', {{ method: 'POST' }})
                .then(r => r.json())
                .then(data => {{ alert('✅ Autoposting atualizado!'); closeSeriesModal(); window.location.reload(); }});
        }}
        ...
        function deleteCollection(mixId) {{
            if (confirm('Deseja excluir esta coleção?')) {{
                fetch(ROOT_PATH + '/api/douyin/collections/' + mixId + '/delete', {{ method: 'POST' }})
                    .then(r => r.json())
                    .then(data => {{ alert('✅ Coleção excluída!'); closeSeriesModal(); window.location.reload(); }});
            }}
        }}
...
print("✅ Web panel v5 recompilado com Template Literals (backticks) no openSeriesModal!")
