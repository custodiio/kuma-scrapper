"""
Script para injetar o Frontend Vitrine Netflix no src/web_panel.py (alvo exato no get_main_html)
"""

import sys, io
from pathlib import Path

# Configura stdout no Windows
...
content = content.replace(
    ...
    'from src import database, search_scrapper, media_processor, drive_uploader, douyin_collection_scraper, episode_scheduler'
)

# 2. Rotas de API para Coleções e Episódios
api_routes = '''
...
async def set_daily_post_rate_api(rate: int = Form(...)):
    ...
    return {"ok": success, "daily_post_rate": rate}

@app.get("/api/status")'''

content = content.replace('@app.get("/api/status")', api_routes)

# 3. Altera a aba padrão para 'collections' na rota index()
...
content = content.replace('tab = "search"', 'tab = "collections"')

# 4. Injeta a aba 'collections' no template HTML
content = content.replace(
...
# 5. Adiciona o bloco de renderização da ABA COLLECTIONS
collections_render = '''    # ─── ABA 0: COLEÇÕES DO DOUYIN (VITRINE NETFLIX) ─────────────────────────
    if tab == "collections":
        daily_rate = episode_scheduler.get_daily_post_rate()
...
# 6. Adiciona os estilos CSS (escapados com {{ }})
netflix_css = '''
...
# 7. Adiciona Modais HTML e Javascript antes da última tag </html> do template principal
modals_js = """
...
# Injeta os modais no final da função get_main_html
content = content.replace('</body>\n</html>\n"""', modals_js + '\n</body>\n</html>\n"""')

panel_path.write_text(content, encoding="utf-8")
print("✅ Frontend Vitrine Netflix injetado com SUCESSO no get_main_html do src/web_panel.py!")
