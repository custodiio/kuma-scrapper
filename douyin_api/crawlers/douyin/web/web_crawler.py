...
class DouyinWebCrawler:
    ...
    # 获取指定关键词的综合搜索结果
    async def fetch_general_search_result(self, keyword: str, offset: int = 0, count: int = 20, sort_type: int = 2, publish_time: int = 0, filter_duration: int = 0):
        kwargs = await self.get_douyin_headers()
        base_crawler = BaseCrawler(proxies=kwargs["proxies"], crawler_headers=kwargs["headers"])
        async with base_crawler as crawler:
            base_params = BaseRequestModel().dict()
            search_params = {
                "keyword": keyword,
                "offset": offset,
                "count": count,
            ...
            base_params["msToken"] = ''
            a_bogus = BogusManager.ab_model_2_endpoint(base_params, kwargs["headers"]["User-Agent"])
            endpoint = f"{DouyinAPIEndpoints.VIDEO_SEARCH}?{urlencode(base_params)}&a_bogus={a_bogus}"
            response = await crawler.fetch_get_json(endpoint)
        return response
