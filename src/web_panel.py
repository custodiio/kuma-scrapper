...
from src import database, search_scrapper, media_processor, drive_uploader, douyin_collection_scraper, episode_scheduler, douyin_profile_scraper
...
def send_telegram_status(text: str, message_id: int = None) -> int:
    ...
    return message_id
def generate_netscape_cookies(cookie_str: str, temp_dir: str) -> str:
    """Converte uma string de cookie no formato HTTP para o formato Netscape usado pelo yt-dlp."""
    cookie_file_path = os.path.join(temp_dir, "bili_cookies.txt")
    try:
        os.makedirs(temp_dir, exist_ok=True)
        with open(cookie_file_path, "w", encoding="utf-8") as f:
            f.write("# Netscape HTTP Cookie File\n")
            f.write("# This file was generated by Kuma Scrapper\n\n")
            ...
            for part in cookie_str.split(";"):
                ...
                # Bilibili usa .bilibili.com como domínio base para cookies
                f.write(f".bilibili.com\tTRUE\t/\tFALSE\t0\t{name}\t{val}\n")
        logger.info(f"Arquivo de cookies do Bilibili gerado em: {cookie_file_path}")
        return cookie_file_path
    except Exception as e:
        logger.error(f"Erro ao gerar cookiefile Netscape: {e}")
        return None
...
def perform_background_download(bvid: str, url: str, is_douyin: bool, category: str, content_type: str):
    ...
    if not is_douyin:
        ...
        try:
            ...
            if bili_cookie:
                cookie_file_path = generate_netscape_cookies(bili_cookie, TEMP_DIR)
                if cookie_file_path:
...
async def add_douyin_collection_api(
    ...
    """Agenda a extração e salvamento de uma nova coleção do Douyin."""
...
async def toggle_autoposting_api(mix_id: str):
    """Alterna a chave de Autoposting (ON/OFF) da coleção."""
...
async def delete_collection_api(mix_id: str):
    """Deleta uma coleção do banco."""
    success = database.delete_douyin_collection(mix_id)
    return {"ok": success}

@app.post("/api/douyin/episodes/{ep_id}/action")
async def episode_action_api(ep_id: int, action: str = Form(...)):
    """Aplica uma ação em 1-clique em um episódio (Postar Agora, Próximo da Fila, Acelerar, etc)."""
...
async def set_daily_post_rate_api(rate: int = Form(...)):
    """Define o ritmo diário de postagens (1, 2 ou 3)."""
...
async def set_autopost_times_api(times: str = Form(...)):
    """Define os horários fixos personalizados fornecidos pelo usuário."""
...
async def get_cookie_api():
    """Retorna o cookie do Douyin atualmente salvo no .env."""
...
async def save_cookie_api(cookie: str = Form(...)):
    """Salva o DOUYIN_COOKIE no .env e sincroniza automaticamente."""
    ...
    try:
        ...
        return {"ok": True, "message": "Cookie do Douyin salvo no .env e sincronizado com sucesso!"}
    except Exception as e:
        return {"ok": False, "message": f"Erro ao salvar cookie: {e}"}

# --- ROTAS DE API PARA PERFIS DO DOUYIN ---

@app.get("/api/douyin/profiles")
async def get_douyin_profiles_api():
    """Retorna todos os perfis do Douyin monitorados."""
...
async def add_douyin_profile_api(url: str = Form(...)):
    """Cadastra um novo perfil do Douyin e mapeia postagens dos últimos 2 meses."""
...
async def delete_douyin_profile_api(sec_uid: str):
    """Deleta um perfil monitorado do Douyin."""
...
async def sync_all_content_api(background_tasks: BackgroundTasks):
    """Dispara a varredura autônoma de atualizações de coleções e perfis (08:00, 12:00, 18:00)."""
    background_tasks.add_task(douyin_profile_scraper.sync_all_profiles_and_collections)
    return {"ok": True, "message": "Varredura autônoma de atualizações iniciada em background (08:00 / 12:00 / 18:00)."}
...
<body>
    ...
    <script>
        ...
        function openSeriesModal(mixId) {
            fetch(ROOT_PATH + '/api/douyin/collections/' + mixId)
                .then(r => r.json())
                .then(data => {
                    ...
                    document.getElementById('modalHeroContent').innerHTML =
                        ...
                        '<div class="modal-hero-info">' +
                            ...
                            '<p style="color:#aaa; margin:0 0 12px 0; font-size:0.9rem;">' + (c.title_zh || '') + ' • 👤 Autor: ' + c.author + '</p>' +
        ...
        function toggleAutoposting(mixId) {
            fetch(ROOT_PATH + '/api/douyin/collections/' + mixId + '/toggle-autoposting', { method: 'POST' })
                .then(r => r.json())
        ...
        function deleteCollection(mixId) {
            if (confirm('Tem certeza que deseja excluir esta coleção?')) {
                fetch(ROOT_PATH + '/api/douyin/collections/' + mixId + '/delete', { method: 'POST' })
                    .then(r => r.json())
...
async def index(
    ...
    # ─── ABA 0: COLEÇÕES DO DOUYIN (VITRINE NETFLIX) ─────────────────────────
    if tab == "collections":
        daily_rate = episode_scheduler.get_daily_post_rate()
        ...
        <div style="display:flex; gap:12px; align-items:center;">
            ...
            <select id="dailyPostRateSelect" onchange="updateDailyPostRate(this.value)" style="background:#1a1d24; color:#fff; border:1px solid #333; padding:8px 12px; border-radius:8px; font-weight:bold; cursor:pointer;">
                <option value="1" {opt1}>1 vídeo / dia</option>
                <option value="2" {opt2}>2 vídeos / dia (Recomendado)</option>
                <option value="3" {opt3}>3 vídeos / dia</option>
            </select>
            <button class="btn-sync" onclick="openCookieModal()" style="background: #374151;">🍪 Cookie Douyin</button>
            <button class="btn-sync" onclick="openAddProfileModal()" style="background: #1e3a8a;">👤 Cadastrar Perfil</button>
            <button class="btn-sync" onclick="syncAllContentNow()" style="background: #065f46;">🔄 Varredura (08h/12h/18h)</button>
...
<body>
    ...
    <!-- MODAL CADASTRO DE PERFIL DOUYIN -->
    <div id="addProfileModal" class="modal-overlay">
        <div class="modal-container" style="max-width: 520px; padding: 24px;">
            <span class="modal-close" onclick="closeAddProfileModal()">&times;</span>
            <h3 style="margin-top:0; font-size:1.3rem;">👤 Cadastrar Perfil Douyin</h3>
            <p style="color:#888; font-size:0.88rem; margin-bottom:20px;">Cole a URL do perfil do Douyin (ex: https://www.douyin.com/user/MS4w...) para mapear as postagens dos últimos 2 meses.</p>
            <form id="addProfileForm" onsubmit="submitAddProfile(event)">
                <div style="margin-bottom: 20px;">
                    <label style="display:block; font-size:0.85rem; color:#ccc; margin-bottom:6px; font-weight:bold;">URL ou sec_uid do Perfil:</label>
                    <input type="text" id="profileUrlInput" placeholder="Ex: https://www.douyin.com/user/MS4wLjABAAAA..." required style="width:100%; padding:10px 14px; background:#1a1d24; border:1px solid #333; color:#fff; border-radius:8px;">
                </div>
                <button type="submit" id="btnAddProfileSubmit" class="btn-sync" style="width:100%; background: linear-gradient(135deg, #1e3a8a, #1e40af); padding: 12px;">📡 Mapear Postagens (Últimos 2 Meses)</button>
            </form>
        </div>
    </div>

    <!-- MODAL CONFIGURAÇÃO DE COOKIE DOUYIN -->
    <div id="cookieModal" class="modal-overlay">
        <div class="modal-container" style="max-width: 600px; padding: 24px;">
            ...
            <h3 style="margin-top:0; font-size:1.3rem;">🍪 Configurar Cookie do Douyin</h3>
            <p style="color:#888; font-size:0.88rem; margin-bottom:16px;">Cole o cookie extraído do seu navegador. Ele será salvo no arquivo <code>.env</code> e sincronizado automaticamente com a API local do Evil0ctal.</p>
            <form id="cookieForm" onsubmit="submitSaveCookie(event)">
                <div style="margin-bottom: 20px;">
                    <label style="display:block; font-size:0.85rem; color:#ccc; margin-bottom:6px; font-weight:bold;">Valor do DOUYIN_COOKIE:</label>
                    <textarea id="cookieInput" rows="6" placeholder="Cole o cookie aqui..." required style="width:100%; padding:10px; background:#1a1d24; border:1px solid #333; color:#fff; border-radius:8px; font-family:monospace; font-size:0.82rem;"></textarea>
                </div>
                <button type="submit" id="btnSaveCookie" class="btn-sync" style="width:100%; background: linear-gradient(135deg, #0288d1, #01579b); padding: 12px;">💾 Salvar e Sincronizar Cookie</button>
            </form>
        </div>
    </div>
    ...
    <script>
        function openCookieModal() {{
            fetch(ROOT_PATH + '/api/douyin/settings/cookie')
                .then(r => r.json())
                .then(data => {{
        ...
        function submitSaveCookie(e) {{
            e.preventDefault();
            const btn = document.getElementById('btnSaveCookie');
            btn.disabled = true;
            btn.innerText = '💾 Salvando e Sincronizando...';

            const formData = new FormData();
            formData.append('cookie', document.getElementById('cookieInput').value);

            fetch(ROOT_PATH + '/api/douyin/settings/cookie', {{ method: 'POST', body: formData }})
                .then(r => r.json())
                .then(data => {{
                    btn.disabled = false;
                    btn.innerText = '💾 Salvar e Sincronizar Cookie';
                    if (data.ok) {{
                        alert('✅ ' + data.message);
                        closeCookieModal();
                    }} else {{
                ...
                .catch(err => {{
                    btn.disabled = false;
                    btn.innerText = '💾 Salvar e Sincronizar Cookie';
                    alert('❌ Erro ao salvar cookie: ' + err);
                }});
        }}
        ...
        function submitAddProfile(e) {{
            ...
            btn.disabled = true;
            btn.innerText = '📡 Mapeando Perfil... Aguarde...';
            ...
            fetch(ROOT_PATH + '/api/douyin/profiles/add', {{ method: 'POST', body: formData }})
                ...
                .then(data => {{
                    btn.disabled = false;
                    btn.innerText = '📡 Mapear Postagens (Últimos 2 Meses)';
                    if (data.ok) {{
                        alert('✅ ' + data.message);
                ...
                .catch(err => {{
                    btn.disabled = false;
                    btn.innerText = '📡 Mapear Postagens (Últimos 2 Meses)';
                    alert('❌ Erro ao mapear perfil: ' + err);
                }});
        }}

        function syncAllContentNow() {{
            if (confirm('Deseja iniciar a varredura autônoma de atualizações de coleções e perfis agora?')) {{
